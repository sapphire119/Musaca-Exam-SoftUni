namespace SIS.MvcFramework.Routing
{
	...
	public class RoutingEngine
    {
        ...
        private static void RegisterStaticFiles(
            ...
            MvcFrameworkSettings settings)
        {
			var path = FileRelativePath + settings.WwwrootPath;
            ...
            foreach (var file in files)
            {
				...
				var url = file.Replace(FileRelativePath, string.Empty)
							  .Replace("\\", "/")
							  .Replace(settings.WwwrootPath, string.Empty);

				if (url.StartsWith("/css"))
				{
					url = url.Substring("/css".Length);
				}
				else if (url.StartsWith("/js"))
				{
					url = url.Substring("/js".Length);
				}
