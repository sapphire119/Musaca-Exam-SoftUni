...
namespace SIS.MvcFramework
{
    public abstract class Controller
    {
        ...
        protected IHttpResponse BadRequestErrorWithView<T>(string errorMessage, T model, string layoutName = "_Layout")
        {
			...
			var layoutFileContent = System.IO.File.ReadAllText(FileRealtivePath + $"Views/{layoutName}.html");
        ...
        {
			var content = this.ViewEngine.GetHtml(
				viewName,
				System.IO.File.ReadAllText(FileRealtivePath + "Views/" + viewName + ".html"), 
				model, 
				this.User);

            if (layoutName != null)
            {
				var layoutFileContent = System.IO.File.ReadAllText(
					FileRealtivePath + $"Views/{layoutName}.html");
